...
def main():
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        lr = cv2.resize(lr, dsize=(1920, 1200))
        ...
        if opt.get("concat_histeq", False):
            his = cv2.resize(his, dsize=(1920, 1200))
            his = t(his)
        ...
        sr = cv2.resize(sr, dsize=(w, h))
        print(sr.shape)
        #sr = sr[:h * scale, :w * scale]

        path_out_sr = os.path.join(test_dir, "{:0.2f}".format(heat).replace('.', ''), os.path.basename(lr_path))
